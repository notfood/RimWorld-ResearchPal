...
namespace FluffyResearchTree
{
    public class ResearchNode : Node
    {
        ...
        public int Matches( string query )
        {
            ...
            if ( Research.LabelCap.ToString().ToLower(culture).Contains( query ) )
                return 1;
            if ( Research.GetUnlockDefsAndDescs().Any( unlock => unlock.First.LabelCap.ToString().ToLower(culture).Contains( query ) ) )
                return 2;
